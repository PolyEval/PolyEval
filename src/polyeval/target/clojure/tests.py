...
from polyeval.target.clojure.value import ClojureTargetValue
...
class ClojureTargetTests(BaseTargetTests):
    def __init__(self):
        self.naming = ClojureTargetNaming()
        self.type = ClojureTargetType()
        self.value = ClojureTargetValue()
...
"""
    ...
    def gen_input_str_cmd(self, params_names: list[str], input_types: list[Type]):
        ...
        return f'(def i-str (str "input: (" {val_str} ")\\n"))'

    def gen_side_str_cmd(self, params_names: list[str], input_types: list[Type]):
        val_str = ' ", " '.join(
            [self.stringify.get(t, name) for name, t in zip(params_names, input_types)]
        )
        return f'(def s-str (str "side-effect: (" {val_str} ")\\n"))'
