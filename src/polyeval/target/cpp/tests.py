...
from polyeval.target.cpp.value import CPPTargetValue
...
class CPPTargetTests(BaseTargetTests):
    def __init__(self):
        self.naming = CPPTargetNaming()
        self.type = CPPTargetType()
        self.value = CPPTargetValue()
...
"""
    ...
    def gen_input_str_cmd(self, params_names: list[str], input_types: list[Type]):
        ...
        return f'auto i_str = "input: (" + {val_str} + ")\\n";'

    def gen_side_str_cmd(self, params_names: list[str], input_types: list[Type]):
        val_str = ' + ", " + '.join(
            [self.stringify.get(t, name) for name, t in zip(params_names, input_types)]
        )
        return f'auto s_str = "side-effect: (" + {val_str} + ")\\n";'
