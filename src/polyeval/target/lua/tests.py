...
from polyeval.target.lua.value import LuaTargetValue
...
class LuaTargetTests(BaseTargetTests):
    def __init__(self):
        self.naming = LuaTargetNaming()
        self.type = LuaTargetType()
        self.value = LuaTargetValue()
...
"""
    ...
    def gen_assign_vars_cmds(
        ...
        for name, value in zip(params_names, test_inputs):
            result.append(f"local {name} = {self.value.by(value)}")
        return "\n".join(result)
