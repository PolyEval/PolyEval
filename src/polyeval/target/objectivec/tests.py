...
from polyeval.target.objectivec.value import ObjectiveCTargetValue
...
class ObjectiveCTargetTests(BaseTargetTests):
    def __init__(self):
        self.naming = ObjectiveCTargetNaming()
        self.type = ObjectiveCTargetType()
        self.value = ObjectiveCTargetValue()
    ...
    def gen_code(self, funcs: list[Function]):
        ...
        for func_signature, func_tests in tests:
            ...
            for test_item_name, test_item_code in func_tests:
                test_entry_ret += f"[self {test_item_name}], "
                result += test_item_code
        test_entry_ret += '@"\\n"'
        test_entry = f"""\
+ (NSString *)p_e_entry {{
    return [@[{test_entry_ret}] componentsJoinedByString:@""];
}}
"""
...
+ (NSString *) {test_item_func_name} {{
    ...
    return [@[@"-\\n", iStr, oStr, eStr, sStr] componentsJoinedByString:@""];
}}
...
"""
    ...
    def gen_assign_vars_cmds(
        ...
        for name, value in zip(params_names, test_inputs):
            result.append(
                f"{self.type.by(value.type)} {name} = {self.value.by(value)};"
            )
        return "\n".join(result)
