...
from polyeval.target.purescript.value import PureScriptTargetValue
...
class PureScriptTargetTests(BaseTargetTests):
    def __init__(self):
        self.naming = PureScriptTargetNaming()
        self.type = PureScriptTargetType()
        self.value = PureScriptTargetValue()
...
"""
    ...
    def gen_function_test(
        ...
        test_code = f"""\
{test_item_func_name} =
    let {add_indent(self.gen_assign_vars_cmds(params_names, test_inputs), 2)}
