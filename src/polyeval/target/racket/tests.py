...
from polyeval.target.racket.value import RacketTargetValue
...
class RacketTargetTests(BaseTargetTests):
    def __init__(self):
        self.naming = RacketTargetNaming()
        self.type = RacketTargetType()
        self.value = RacketTargetValue()
...
"""
    ...
    def gen_assign_vars_cmds(
        ...
        for name, value in zip(params_names, test_inputs):
            result.append(f"(define {name} {self.value.by(value)})")
        return "\n".join(result)
