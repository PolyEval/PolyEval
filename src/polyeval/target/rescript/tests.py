...
from polyeval.target.rescript.type import ReScriptTargetType
from polyeval.target.rescript.value import ReScriptTargetValue
from polyeval.target.rescript.naming import ReScriptTargetNaming
from polyeval.target.rescript.value_stringify import ReScriptTargetValueStringify

import json


class ReScriptTargetTests(BaseTargetTests):
    def __init__(self):
        self.naming = ReScriptTargetNaming()
        self.type = ReScriptTargetType()
        self.value = ReScriptTargetValue()
        self.stringify = ReScriptTargetValueStringify()
        pass
...
"""
    ...
    def gen_assign_vars_cmds(
        ...
        for name, value in zip(params_names, test_inputs):
            result.append(f"let {name} = {self.value.by(value)};")
        return "\n".join(result)
