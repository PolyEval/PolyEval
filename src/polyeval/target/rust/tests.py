...
from polyeval.target.rust.value import RustTargetValue
...
fn {test_item_func_name}() -> String {{
    ...
    {self.gen_output_cmd(target_func_name, params_names, input_types)}
    {self.gen_output_str_cmd(func_ret_type)}
    {self.gen_side_str_cmd(params_names, input_types)}
    "-\\n".to_string() + &i_str + &o_str + &e_str + &s_str
}}
...
"""
    ...
    def gen_assign_vars_cmds(
        ...
        for name, value in zip(params_names, test_inputs):
            result.append(
                f"let mut {name}: {self.type.by(value.type)} = {self.value.by(value)};"
            )
        return "\n".join(result)
    ...
    def gen_input_str_cmd(self, params_names: list[str], input_types: list[Type]):
        ...
        return f'let i_str = "input: (".to_string() + &{val_str} + ")\\n";'

    def gen_side_str_cmd(self, params_names: list[str], input_types: list[Type]):
        val_str = ' + ", " + &'.join(
            [self.stringify.get(t, name) for name, t in zip(params_names, input_types)]
        )
        return f'let s_str = "side-effect: (".to_string() + &{val_str} + ")\\n";'

    def gen_output_cmd(
        self, func_name: str, params_names: list[str], input_types: list[Type]
    ):
        new_params_names = [
            f"&{name}"
            if isinstance(
            ...
            for name, t in zip(params_names, input_types)
        ]
        return f"let res = {func_name}({', '.join(new_params_names)});"

    def gen_output_str_cmd(self, t: Type):
        val_str = self.stringify.get(t, "res")
        return f'let o_str = "output: ".to_string() + &{val_str} + "\\n";'

    def gen_expected_str_cmd(self, v: TypedValue):
        val_str = self.stringify.get(v.type, self.value.by(v))
        return f'let e_str = "expected: ".to_string() + &{val_str} + "\\n";'
