...
from polyeval.target.typescript.value import TypeScriptTargetValue
...
class TypeScriptTargetTests(BaseTargetTests):
    def __init__(self):
        self.naming = TypeScriptTargetNaming()
        self.type = TypeScriptTargetType()
        self.value = TypeScriptTargetValue()
...
"""
    ...
    def gen_function_test(
        ...
        test_code = f"""\
function {test_item_func_name}(): string {{
...
"""
    ...
    def gen_assign_vars_cmds(
        ...
        for name, value in zip(params_names, test_inputs):
            result.append(
                f"let {name}: {self.type.by(value.type)} = {self.value.by(value)};"
            )
        return "\n".join(result)
